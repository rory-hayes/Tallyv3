...
describe("pack generation and locking", () => {
  ...
  it("applies redaction settings to pack content", async () => {
    const { firm, user, payRun } = await seedApprovedPayRun();
    ...
    await prisma.exception.update({
      ...
      data: {
        title: "Employee: a Smith Account 12345678 NI AB123456C",
        evidence: [{ note: "No rows" }]
      }
    });
    ...
    expect(pdfText).toContain("Employee: a S****");
    expect(pdfText).toContain("****5678");
    expect(pdfText).toContain("*******6C");
    expect(pdfText).not.toContain("Employee: a Smith");
  ...
  it("errors when generated by user is missing", async () => {
    const { firm, payRun } = await seedApprovedPayRun();
    const missingUserId = randomUUID();

    await expect(
      generatePack(
        { firmId: firm.id, userId: missingUserId, role: "ADMIN" },
        payRun.id
      )
    ).rejects.toThrow("Generated by user not found.");
  });

  it("omits delta text when check details are non-numeric", async () => {
    const { firm, user, payRun } = await seedApprovedPayRun();
    const run = await prisma.reconciliationRun.findFirst({
      where: { payRunId: payRun.id }
    });
  ...
  it("derives storage keys from URIs when keys are missing", async () => {
    const { firm, user, payRun } = await seedApprovedPayRun();
    vi.spyOn(storageClient as unknown as { send: () => Promise<unknown> }, "send")
      .mockResolvedValueOnce({});
    ...
    const url = await getPackDownloadUrl({
      ...pack,
      storageKeyPdf: null
    } as Pack);
    expect(url).toContain("X-Amz-");
